...
class GCPManager:
    ...
    def create_instance(
        ...
        network_interface.network = "global/networks/default"  # Changed from name to network
    ...
    def get_instance(self, name: str, zone: str) -> Optional[GCPInstance]:
        ...
        try:
            ...
            for nic in instance.network_interfaces:
                # Use the correct field name for internal IP
                if hasattr(nic, 'network_ip'):
                    internal_ips.append(nic.network_ip)
                elif hasattr(nic, 'ip_address'):
                    internal_ips.append(nic.ip_address)
                
                # Extract external IPs from access configs
                for config in nic.access_configs:
                    if hasattr(config, 'nat_ip') and config.nat_ip:
                        external_ips.append(config.nat_ip)
                    elif hasattr(config, 'external_ip') and config.external_ip:
                        external_ips.append(config.external_ip)
...
def debug_api_error(e: Exception) -> str:
    """Format API error details for better debugging."""
    error_details = str(e)
    
    if isinstance(e, GoogleAPIError):
        error_details = f"API Error: {e.message}"
...
def main():
    ...
    parser.add_argument("--project", help="GCP Project ID", default="aerial-rarity-432619-j5")
    ...
    if args.debug:
        ...
        logging.info("Debug mode enabled - showing detailed error messages")
    ...
    try:
        ...
        # Test API connection
        logging.info(f"Testing connection to GCP API with project: {project_id}")
        try:
            # Try a simple API call to verify credentials and project access
            regions = manager.get_regions()
            logging.info(f"Successfully connected to GCP API. Found {len(regions)} regions.")
        except Exception as e:
            logging.error(f"Failed to connect to GCP API: {debug_api_error(e)}")
            print("\nError: Could not connect to Google Cloud API. Please check:")
            print("  1. Your project ID is correct")
            print("  2. You have the necessary permissions")
            print("  3. The Compute Engine API is enabled for your project")
        ...
        while True:
            ...
            try:
                if choice == "0":
                    ...
                    instances = manager.list_instances()
                    if not instances:
                        print("\nNo instances found in your project.")
                    else:
                        print("\nYour Instances:")
                        for instance in instances:
                    ...
                    if not results:
                        print("\nNo instances available for testing.")
                    else:
                ...
                if args.debug:
                    print(f"\nError details: {debug_api_error(e)}")
                else:
                    print(f"\nAn error occurred: {str(e)}")
                    print("Run with --debug flag for more detailed error information.")
    except Exception as e:
        if args.debug:
            print(f"\nFailed to initialize GCP Manager: {debug_api_error(e)}")
        else:
            print(f"\nFailed to initialize GCP Manager: {str(e)}")
            print("Run with --debug flag for more detailed error information.")
