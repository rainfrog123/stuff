...
(function() {
    ...
    const STATE = {
        ...
        resetRoundCounter() {
            this.roundCounter = 0;
            this.roundsToWait = Math.floor(utils.cryptoRandom(CONFIG.ROUNDS.MIN, CONFIG.ROUNDS.MAX));
    ...
    const betting = {
        ...
        calculateBetAmount: () => {
            ...
            const randomBetPercentage = utils.cryptoRandom(CONFIG.BET_PERCENTAGE.MIN, CONFIG.BET_PERCENTAGE.MAX);
            ...
            let betAmount = Math.round(utils.cryptoRandom(CONFIG.MIN_BET, maxBet) * 10) / 10;
        ...
        chooseBetType: () => {
            // Simple 50:50 choice
            const choice = utils.cryptoRandom(0, 1) < 0.5 ? "Player" : "Banker";
            ...
            try {
                ...
                if (remainingAmount >= CONFIG.CHIPS.MEDIUM.VALUE) {
                    ...
                    if (mediumChipElement) {
                        ...
                        await new Promise(resolve => setTimeout(resolve, utils.cryptoRandom(CONFIG.DELAYS.CLICK.MIN, CONFIG.DELAYS.CLICK.MAX)));
        ...
        handleBet: () => {
            ...
            // Use variable delay
            const delay = utils.cryptoRandom(CONFIG.DELAYS.BET.MIN, CONFIG.DELAYS.BET.MAX);
        ...
        if (["B", "T", "P"].includes(name)) {
            ...
            if (STATE.waitingForResult && STATE.lastBet) {
                ...
                if (Math.random() < 0.3) { // 30% chance
                    window.crypto.getRandomValues(STATE.entropyPool);
            ...
            // Use variable delay with cryptoRandom
            const betDelay = 300 + Math.floor(utils.cryptoRandom(0, 400));
    ...
    function initialize() {
        ...
        window.crypto.getRandomValues(STATE.entropyPool);
        window.crypto.getRandomValues(STATE.lastRandomValues);
    }
