// ==UserScript==
// @name         Baccarat Simple v3 - Event-Driven Dynamic Betting
// @namespace    http://tampermonkey.net/
// @version      5.0
// @description  Event-driven crypto-secure dynamic betting - 1/8-1/2 balance every 1-3 rounds (receives comprehensive game state from result.js)
// @author       you
...
(function () {
    ...
    class Logger {
        static log(msg) {
            console.log(`[DynamicBet] ${new Date().toLocaleTimeString()} - ${msg}`);
        }
        static debug(msg) {
    ...
    class GameState {
        ...
        start() {
            ...
            Logger.log('🟢 Started - Dynamic betting 1/8-1/2 balance every 1-3 rounds (Event-driven)');
            this.isRunning = true;
    ...
    class MiniHUD {
        ...
        update() {
            ...
            this.hud.innerHTML = `${status} #${s.round} | Next: ${nextBet} | ${balance}${lastResult}${counters}${winRate} | Dynamic Bet`;
        }
    }
    ...
    setTimeout(() => {
        if (DOMUtils.areElementsReady()) {
            gameState.start();
            Logger.log('✅ DynamicBet v5.0 ready - Event-driven dynamic betting enabled');
    ...
    window.dynamicBet = {
        ...
        calcBet: () => {
            ...
            console.log(`Balance: $${balance.toFixed(2)}, Dynamic Bet: $${result.betAmount.toFixed(2)} (${result.chips} chips)`);
            return result;
        },
