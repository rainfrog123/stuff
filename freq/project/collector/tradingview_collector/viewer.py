...
class DataViewer:
    ...
    def get_recent_candles(self, symbol: str, count: int = 20) -> pd.DataFrame:
        ...
        try:
            # Use the database's connection context manager
    ...
    def show_overview(self):
        ...
        print(f"=== 5-Second Candle Database Overview ===")
        print(f"Database: {self.db.candles_db_path}\n")
        ...
        for symbol in SYMBOLS:
            ...
            if 'error' not in stats:
                stats_data.append([
                    ...
                    stats['max_time'].strftime('%Y-%m-%d %H:%M:%S') if stats['max_time'] else "N/A"
                ])
            else:
