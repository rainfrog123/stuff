...
class freqai_opaq_classifier(IStrategy):
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        # Set classification target: up, down, or same
        future_price = dataframe["close"].shift(-self.freqai_info["feature_parameters"]["label_period_candles"])
        dataframe["&-class"] = (
            future_price > dataframe["close"]
        ).astype(int)  # 1 for up, 0 for down/same
        return dataframe
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        enter_long_conditions = [
            df["do_predict"] == 1,
            df["&-class"] == 1,  # class 1 means up (predicted price increase)
        ]
        ...
        enter_short_conditions = [
            df["do_predict"] == 1,
            df["&-class"] == 0,  # class 0 means down (predicted price decrease)
        ]
