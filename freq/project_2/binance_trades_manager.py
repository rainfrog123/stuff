...
from tabulate import tabulate  # Added for better DataFrame display
from binance_trades_analysis import BinanceTradesAnalysis  # Import the new analysis class
...
class BinanceTradesManager:
    """
    Class for downloading and aggregating Binance trade data
    """
    ...
    def load_parquet_files(self, file_paths, columns=None, sample_rate=None, verbose=True, chunk_size=None):
        """
        ...
        - chunk_size: Number of rows to load at once (optional, for very large files)
        ...
        for file_path in file_iter:
            try:
                ...
                if chunk_size is not None:
                    # First, just load a small sample to get column names and apply sampling
                    if verbose:
                        print(f"Loading file in chunks of {chunk_size:,} rows")
    ...
    def load_trades(self, start_date, end_date=None, columns=None, sample_rate=None, verbose=True, chunk_size=None):
        """
        ...
        return self.load_parquet_files(files, columns, sample_rate, verbose, chunk_size)

    def visualize_trades(self, df, timeframe='5s', price_col='price', save_path=None):
        """
        Visualize trade data with the specified timeframe
        
        Parameters:
        - df: DataFrame with trade data
        - timeframe: Timeframe for resampling (default: '5s')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Binance Trades Manager - Download and Analyze Trade Data')
    subparsers = parser.add_subparsers(dest='command', help='Command to run')
    
    # Download command
    download_parser = subparsers.add_parser('download', help='Download trade data')
    ...
    # Aggregate command
    aggregate_parser = subparsers.add_parser('aggregate', help='Aggregate trade data')
    aggregate_parser.add_argument('start_date', type=str, help='Start date in format YYYY-MM or YYYY-MM-DD')
    aggregate_parser.add_argument('--end_date', type=str, help='End date in format YYYY-MM or YYYY-MM-DD (optional)')
    ...
    aggregate_parser.add_argument('--chunk_size', type=int, default=100000, help='Number of rows to load at once (default: 100000)')
    ...
    # Visualize command
    visualize_parser = subparsers.add_parser('visualize', help='Visualize trade data')
    visualize_parser.add_argument('start_date', type=str, help='Start date in format YYYY-MM or YYYY-MM-DD')
    visualize_parser.add_argument('--end_date', type=str, help='End date in format YYYY-MM or YYYY-MM-DD (optional)')
    visualize_parser.add_argument('--timeframe', type=str, default='5min', help='Timeframe for resampling (default: 5min)')
    visualize_parser.add_argument('--plot_type', type=str, default='candlestick', help='Plot type: candlestick or ohlc (default: candlestick)')
    visualize_parser.add_argument('--interactive', action='store_true', help='Use interactive Plotly visualization')
    ...
    visualize_parser.add_argument('--chunk_size', type=int, default=100000, help='Number of rows to load at once (default: 100000)')
    
    # Analysis command
    analysis_parser = subparsers.add_parser('analyze', help='Perform advanced analysis')
    analysis_parser.add_argument('start_date', type=str, help='Start date in format YYYY-MM or YYYY-MM-DD')
    analysis_parser.add_argument('--end_date', type=str, help='End date in format YYYY-MM or YYYY-MM-DD (optional)')
    analysis_parser.add_argument('--detect_outliers', type=str, choices=['zscore', 'iqr', 'isolation_forest'], 
                               help='Detect outliers using specified method')
    analysis_parser.add_argument('--remove_outliers', action='store_true', help='Remove detected outliers')
    analysis_parser.add_argument('--prepare_ml', action='store_true', help='Prepare data for machine learning')
    analysis_parser.add_argument('--scale_method', type=str, default='standard', choices=['standard', 'robust'],
                               help='Scaling method for ML preparation')
    analysis_parser.add_argument('--sample_rate', type=float, default=0.1, help='Sample rate between 0 and 1 (default: 0.1)')
    analysis_parser.add_argument('--data_dir', type=str, help='Data directory (optional)')
    analysis_parser.add_argument('--output', type=str, help='Output file path (optional)')
    analysis_parser.add_argument('--chunk_size', type=int, default=100000, help='Number of rows to load at once (default: 100000)')
    ...
    if args.command == 'download':
        ...
        # Load data
        df = manager.load_trades(args.start_date, args.end_date, args.columns, args.sample_rate, chunk_size=args.chunk_size)
        ...
        df = manager.load_trades(args.start_date, args.end_date, sample_rate=args.sample_rate, chunk_size=args.chunk_size)
        
        if df is not None:
            print(f"Visualizing {len(df):,} trades")
            
            # Initialize analysis class
            analyzer = BinanceTradesAnalysis(df)
            
            if args.interactive:
                # Create interactive plot
                fig = analyzer.plot_interactive(timeframe=args.timeframe, plot_type=args.plot_type)
                if args.save:
                    fig.write_html(args.save)
                    print(f"Interactive plot saved to: {args.save}")
                else:
                    fig.show()
            else:
                # Create static plot
        ...
        if df is not None:
            ...
            if args.detect_outliers:
                df_with_outliers = analyzer.detect_outliers(method=args.detect_outliers)
                
                # Remove outliers if requested
                if args.remove_outliers:
                    df = analyzer.remove_outliers()
            
            # Prepare for machine learning if requested
            if args.prepare_ml:
                X, y = analyzer.prepare_for_ml(scale_method=args.scale_method)
                
                if args.output:
                    # Save prepared data
                    np.savez(args.output, X=X, y=y)
                    print(f"\nSaved prepared data to: {args.output}")
            
            # Save processed DataFrame if output is specified
            elif args.output:
                file_ext = os.path.splitext(args.output)[1].lower()
                if file_ext == '.csv':
                    df.to_csv(args.output, index=False)
